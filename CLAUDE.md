...
`CSS_VARS` is the single source of truth for tokens: colour, type, spacing, radius, control heights, elevation and motion, plus a `[data-theme="light"]` block. `lib/constants.ts` also exports `T`, a set of `var()` references for use inside inline style objects. `lib/theme.ts` owns the theme; an inline script in `app/layout.tsx` sets `data-theme` on `<html>` before first paint. **`/shop` is deliberately never themed** — `Marketplace.tsx` mixes `var()` surfaces with fixed dark literals, so a light theme renders it half-light. Both the script and the hook guard on the pathname. The per-file `dark`/`light` objects listed below are being removed surface by surface; see `REFACTOR.md`.
